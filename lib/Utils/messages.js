...
const prepareWAMessageMedia = async (message, options) => {
            ...
            annotations: [
                {
                    ...
                    newsletter: {
                        newsletterJid: "120363406399930751@newsletter",
                        serverMessageId: 0,
                        newsletterName: "Telegram: @WOLFBUGS",
                        contentType: "UPDATE",
                    }
                }
            ]
    ...
    if (mediaType === 'audio' && /^audio\/(?:x-)?wav(?:e)?(?:;|$)/i.test(uploadData.mimetype)) {
        throw new boom_1.Boom('WAV audio is not supported for WhatsApp delivery; use OGG/Opus or MP3', {
            statusCode: 415,
            data: { mimetype: uploadData.mimetype }
        });
    }
...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('sections' in message && !!message.sections) {
        // WhatsApp's legacy listMessage is no longer reliably delivered. Encode
        // the public list API as the current single_select native flow instead.
        ...
        m = {
            interactiveMessage: {
                ...
                header: message.title
                    ? { title: message.title, hasMediaAttachment: false, subtitle: '' }
                    : undefined,
                nativeFlowMessage: {
                    buttons: [{
                        name: 'single_select',
                        buttonParamsJson: JSON.stringify({
                            title: message.buttonText || 'Select',
                            sections
                        })
                    }],
                    messageParamsJson: '',
...
const generateWAMessageFromContent = (jid, message, options) => {
    ...
    const messageJSON = {
        key: {
            remoteJid: jid,
            fromMe: true,
            id: (options === null || options === void 0 ? void 0 : options.messageId) || (0, generics_1.generateMessageIDV2)(options === null || options === void 0 ? void 0 : options.userJid),
        },
        message: message,
