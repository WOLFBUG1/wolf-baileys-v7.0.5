...
const isAppStateSyncIrrecoverable = (error, attempts) => {
    return attempts >= 2 || (error === null || error === void 0 ? void 0 : error.name) === 'TypeError';
};
...
const decodeSyncdMutations = async (msgMutations, initialState, getAppStateSyncKey, onMutation, validateMacs) => {
    ...
    for (const msgMutation of msgMutations) {
        ...
        try {
            ...
            if ((0, exports.isMissingKeyError)(error)) {
                throw error;
            }
            continue;
        }
...
const decodeSyncdSnapshot = async (name, snapshot, getAppStateSyncKey, minimumVersionNumber, validateMacs = true, logger) => {
    ...
    if (validateMacs) {
        ...
        if (Buffer.compare(snapshot.mac, computedSnapshotMac) !== 0) {
            logger === null || logger === void 0 ? void 0 : logger.warn({ name, version: newState.version }, 'LTHash verification failed on snapshot, continuing with partial state');
        }
    }
    return {
        state: newState,
...
const decodePatches = async (name, syncds, initial, getAppStateSyncKey, options, minimumVersionNumber, logger, validateMacs = true) => {
    ...
    for (let i = 0; i < syncds.length; i++) {
        ...
        try {
            ...
            logger === null || logger === void 0 ? void 0 : logger.warn({ name, version: patchVersion, error: error === null || error === void 0 ? void 0 : error.message }, 'failed to decode patch, skipping');
            continue;
        }
        ...
        if (validateMacs) {
            ...
            if (Buffer.compare(snapshotMac, computedSnapshotMac) !== 0) {
                logger === null || logger === void 0 ? void 0 : logger.warn({ name, version: newState.version }, 'LTHash verification failed, skipping remaining patches');
                break;
            }
        }
