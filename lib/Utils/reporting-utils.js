...
const compileReportingFields = (fields) => {
    ...
    for (const f of fields) {
        map.set(f.f, { m: f.m, children: f.s ? compileReportingFields(f.s) : undefined });
    }
    return map;
};
...
const shouldIncludeReportingToken = (message) => !(message === null || message === void 0 ? void 0 : message.reactionMessage) &&
    !(message === null || message === void 0 ? void 0 : message.encReactionMessage) &&
    !(message === null || message === void 0 ? void 0 : message.encEventResponseMessage) &&
...
const extractReportingTokenContent = (data, cfg) => {
    ...
    while (i < data.length) {
        ...
        if (wireType === WIRE.FIXED64 || wireType === WIRE.FIXED32) {
            const end = i + (wireType === WIRE.FIXED64 ? 8 : 4);
            if (!fieldCfg) {
                if (!skip(end)) {
                    return null;
                }
        ...
        if (wireType === WIRE.BYTES) {
            ...
            if (fieldCfg.m || fieldCfg.children) {
                ...
                if (sub.length > 0) {
                    out.push({ num: fieldNum, bytes: Buffer.concat([encodeVarint(tag.value), encodeVarint(sub.length), sub]) });
                }
                i = valEnd;
                continue;
            }
...
const stripRandomPadMax16 = (buffer) => {
    if (!buffer.length) {
        return buffer;
    }
    const pad = buffer[buffer.length - 1];
    if (pad < 1 || pad > 15 || pad > buffer.length) {
        return buffer;
    }
...
const getMessageReportingToken = async (msgProtobuf, message, key) => {
    var _a;
    const msgSecret = (_a = message === null || message === void 0 ? void 0 : message.messageContextInfo) === null || _a === void 0 ? void 0 : _a.messageSecret;
    if (!msgSecret || !key.id) {
        return null;
    }
    ...
    const content = extractReportingTokenContent(stripRandomPadMax16(Buffer.from(msgProtobuf)), compiledReportingFields);
