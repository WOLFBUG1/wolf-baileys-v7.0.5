...
exports.LANGUAGE_KEYWORDS = exports.PYTHON_KEYWORDS = exports.JS_KEYWORDS = exports.generateRichMessageContent = exports.generateUnifiedResponseContent = exports.captureUnifiedResponse = exports.generateLatexInlineImageContent = exports.generateLatexImageContent = exports.renderLatexToPng = exports.generateLatexContent = exports.generateCodeBlockContent = exports.generateListContent = exports.generateTableContent = exports.buildBotForwardedMessage = exports.buildRichContextInfo = exports.tokenizeCode = exports.RichSubMessageType = exports.CodeHighlightType = void 0;
...
const escapeXml = (value) => String(value)
    ...
    .replace(/>/g, '&gt;')
    .replace(/"/g, '&quot;');
const normalizeLatexText = (expression) => String(expression)
    ...
    .replace(/_\{([^{}]+)\}/g, '_$1')
    .replace(/\\([a-zA-Z]+)/g, '$1');
const renderLatexToPng = async (latexExpression, options = {}) => {
    let sharp;
    try {
        sharp = require('sharp');
    }
    catch (error) {
        throw new Error('sendLatexImage requires the optional "sharp" dependency or a custom renderLatexToPng function');
    }
    const fontSize = options.fontSize || 38;
    const padding = options.padding || 24;
    const maxWidth = options.width || 960;
    const lineHeight = Math.round(fontSize * 1.45);
    const text = normalizeLatexText(latexExpression || '');
    const estimatedCharsPerLine = Math.max(8, Math.floor((maxWidth - padding * 2) / (fontSize * 0.58)));
    const words = text.split(/\s+/).filter(Boolean);
    const lines = [];
    let current = '';
    for (const word of words.length ? words : [text]) {
        const next = current ? `${current} ${word}` : word;
        if (next.length > estimatedCharsPerLine && current) {
            lines.push(current);
            current = word;
        }
        else {
    ...
    const width = Math.min(maxWidth, Math.max(220, padding * 2 + Math.ceil(Math.max(...lines.map(line => line.length), 1) * fontSize * 0.58)));
    const height = Math.max(96, padding * 2 + lineHeight * lines.length);
    const foreground = options.foreground || '#111827';
    const background = options.background || '#ffffff';
    const fontFamily = options.fontFamily || 'Cambria Math, Cambria, Times New Roman, serif';
    const tspans = lines.map((line, index) => `<tspan x="${padding}" dy="${index === 0 ? 0 : lineHeight}">${escapeXml(line)}</tspan>`).join('');
    const svg = `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  <rect width="100%" height="100%" rx="${options.radius || 18}" fill="${escapeXml(background)}"/>
  <text x="${padding}" y="${padding + fontSize}" fill="${escapeXml(foreground)}" font-size="${fontSize}" font-family="${escapeXml(fontFamily)}">${tspans}</text>
</svg>`;
    const buffer = await sharp(Buffer.from(svg)).png().toBuffer();
    return { buffer, width, height };
};
exports.renderLatexToPng = renderLatexToPng;
const generateLatexImageContent = async (quoted, options, uploadFn, renderLatexToPng) => {
    const submessages = [];
    if (options.headerText) {
        submessages.push({ messageType: RichSubMessageType.TEXT, messageText: options.headerText });
    }
    renderLatexToPng = renderLatexToPng || exports.renderLatexToPng;
    const expressions = await Promise.all((options.expressions || []).map(async (expr) => {
        const { buffer, width, height } = await renderLatexToPng(expr.latexExpression, { ...options.renderOptions, ...expr.renderOptions });
        const uploadResult = await uploadFn(buffer, 'image');
        const imageUrl = uploadResult.url || uploadResult.mediaUrl || uploadResult.directPath;
