...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, cachedGroupMetadata, useCachedGroupMetadata, statusJidList, AI = true, forceLidUserDevices, skipOwnDeviceFanout } = {}) => {
        ...
        if (!(decodedJid === null || decodedJid === void 0 ? void 0 : decodedJid.user) || !decodedJid.server) {
            throw new boom_1.Boom('Invalid WhatsApp recipient JID: expected user@server', {
                statusCode: 400
            });
        }
        ...
        if (participant === true) {
            if (isGroup || isStatus || isNewsletter) {
                throw new boom_1.Boom('participant:true requires a direct recipient JID', { statusCode: 400 });
            }
            participant = { jid: destinationJid };
        }
        if (participant) {
            ...
            const participantJid = typeof participant.jid === 'string' ? participant.jid.trim() : '';
            const decodedParticipant = participantJid ? WABinary_1.jidDecode(participantJid) : undefined;
            if (!(decodedParticipant === null || decodedParticipant === void 0 ? void 0 : decodedParticipant.user) || !decodedParticipant.server) {
                throw new boom_1.Boom('Invalid participant JID: expected user@server', { statusCode: 400 });
            }
            if (participantJid !== participant.jid) {
                participant = { ...participant, jid: participantJid };
            }
            const { user, device, server } = decodedParticipant;
    ...
    const normalizeDirectRecipientTarget = (target) => {
    ...
    const resolveComposedMessageTarget = async (target, options = {}) => {
        target = normalizeDirectRecipientTarget(target);
    ...
    const resolveMessageRecipientOptions = async (options = {}) => {
        let participant = options.participant;
        let statusJidList = options.statusJidList;
        if (participant && typeof participant.jid === 'string') {
            const resolvedParticipant = await resolveComposedMessageTarget(participant.jid, options);
            participant = resolvedParticipant.jid === participant.jid
                ? participant
                : { ...participant, jid: resolvedParticipant.jid };
        }
        if (Array.isArray(statusJidList)) {
            statusJidList = await Promise.all(statusJidList.map(async (jid) => {
                const resolved = await resolveComposedMessageTarget(jid, options);
                return resolved.jid;
            }));
        }
        if (participant === options.participant && statusJidList === options.statusJidList) {
            return options;
        }
        return { ...options, participant, statusJidList };
    };
    const relayMessageWithUsername = async (target, message, options = {}) => {
        // Status broadcasts have their real recipients in statusJidList. Keep this
        // path byte-for-byte compatible with relayMessage: resolving/rebuilding
        // the list here broke callers that already prepared phone-number JIDs.
        if (target === WABinary_1.STORIES_JID) {
            return relayMessage(target, message, options);
        }
        target = normalizeDirectRecipientTarget(target);
        options = await resolveMessageRecipientOptions(options);
        if (typeof target !== 'string' || !target.trim().startsWith('@')) {
            return relayMessage(target, message, options);
        }
        const resolved = await resolveComposedMessageTarget(target, options);
        return relayMessage(resolved.jid, message, {
            ...options,
            ...resolved.relayOptions
        });
    };
    const relayComposedMessage = async (target, message, messageId, options = {}) => {
        options = await resolveMessageRecipientOptions(options);
        const resolved = await resolveComposedMessageTarget(target, options);
        await relayMessage(resolved.jid, message, { ...options, messageId, ...resolved.relayOptions });
        return { message, messageId, ...(resolved.usernameLookup ? { usernameLookup: resolved.usernameLookup } : {}) };
    };
    const rahmi = new xeonDugong(
        Utils_1,
        waUploadToServer,
        relayMessageWithUsername,
        async target => (await resolveComposedMessageTarget(target)).jid
    );
    ...
    const messagesSocket = {
        ...
        assertSessions,
        relayMessage: relayMessageWithUsername,
        messageRetryManager,
        sendReceipt,
        sendReceipts,
        rahmi,
        ...
        sendMessageToUsername: async function (username, content, options = {}) {
            ...
            for (const candidate of orderedCandidates) {
                ...
                try {
                    ...
                    const sent = await messagesSocket.sendMessage(candidate.jid, candidateContent, sendOptions);
                    if (sent) {
                        sent.usernameLookup = {
        ...
        sendMessage: async (jid, content, options = {}) => {
            jid = normalizeDirectRecipientTarget(jid);
            options = await resolveMessageRecipientOptions(options);
            if (content && typeof content === 'object' && 'table' in content) {
                const table = content.table;
                if (!table || typeof table !== 'object') {
                    throw new boom_1.Boom('table content must be an object', { statusCode: 400 });
                }
                return messagesSocket.sendTable(
                    jid,
                    table.title,
                    table.headers,
                    table.rows,
                    options.quoted,
                    { ...options, headerText: table.headerText, footer: table.footer }
                );
            }
            if (content && typeof content === 'object' && 'codeBlock' in content) {
                const codeBlock = content.codeBlock;
                const code = typeof codeBlock === 'string' ? codeBlock : codeBlock === null || codeBlock === void 0 ? void 0 : codeBlock.code;
                if (typeof code !== 'string') {
                    throw new boom_1.Boom('codeBlock must be a string or an object containing code', { statusCode: 400 });
                }
                const codeOptions = typeof codeBlock === 'object' && codeBlock
                    ? {
                        ...options,
                        language: codeBlock.language,
                        title: codeBlock.title,
                        footer: codeBlock.footer
                    }
                    : options;
                return messagesSocket.sendCodeBlock(jid, code, options.quoted, codeOptions);
            }
            if (typeof jid === 'string' && jid.trim().startsWith('@')) {
                return messagesSocket.sendMessageToUsername(jid, content, options);
            }
            ...
            if (messageType) {
                switch(messageType) {
                    ...
                    case 'ALBUM':
                        return await rahmi.handleAlbum(content, jid, quoted, options)
                    case 'EVENT':
                        return await rahmi.handleEvent(content, jid, quoted, options)
                    case 'POLL_RESULT':
                        return await rahmi.handlePollResult(content, jid, quoted, options)
                    case 'GROUP_STORY':
                        return await rahmi.handleGroupStory(content, jid, quoted, options)
                }
            }
