...
const makeMessagesSocket = (config) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                for (const { user, device, server } of devices) {
                    ...
                    const jid = WABinary_1.jidEncode(isMe && isLid ? authState.creds?.me?.lid?.split(':')[0] || user : user, server || (isLid ? 'lid' : 's.whatsapp.net'), device)
    ...
    return {
        ...
        sendLatexImage: async (jid, quoted, options, renderLatexToPng, uploadFn) => {
            const { message, messageId } = await (0, Utils_1.generateLatexImageContent)(quoted, options, uploadFn, renderLatexToPng);
            await relayMessage(jid, message, { messageId });
            return { message, messageId };
        },
        sendLatexInlineImage: async (jid, quoted, options, renderLatexToPng, uploadFn) => {
            const { message, messageId } = await (0, Utils_1.generateLatexInlineImageContent)(quoted, options, uploadFn, renderLatexToPng);
            await relayMessage(jid, message, { messageId });
            return { message, messageId };
        },
