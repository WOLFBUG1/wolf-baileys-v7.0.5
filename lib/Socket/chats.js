    ...
    const onWhatsApp = async (...jids) => {
        ...
        if (results) {
            ...
            const mapped = results.list
                .filter((a) => !!a.contact)
                .map(({ contact, id, lid }) => {
                const normalizedLid = lid ? (lid.includes('@') ? lid : `${lid}@lid`) : undefined;
                if (normalizedLid && id) {
                    lidPnMappings.push({ pn: id, lid: normalizedLid });
                }
                return { jid: id, exists: contact, lid: normalizedLid };
            });
            if (lidPnMappings.length && sock.signalRepository?.lidMapping) {
                await sock.signalRepository.lidMapping.storeLIDPNMappings(lidPnMappings);
            }
            return mapped;
        }
    };
        ...
        await authState.keys.transaction(async () => {
            ...
            while (collectionsToHandle.size) {
                ...
                for (const name of collectionsToHandle) {
                    ...
                    if (state) {
                        state = (0, Utils_1.ensureLTHashStateVersion)(state);
                    ...
                    nodes.push({
                        ...
                        attrs: {
                            ...
                            // return snapshot if syncing from scratch or recovering state
                ...
                for (const key in decoded) {
                    ...
                    try {
                        if (snapshot) {
                            const { state: newState, mutationMap } = await (0, Utils_1.decodeSyncdSnapshot)(name, snapshot, getCachedAppStateSyncKey, initialVersionMap[name], appStateMacVerification.snapshot, logger);
                        ...
                        const logData = {
                            ...
                            statusCode: error === null || error === void 0 ? void 0 : error.output?.statusCode,
                            errorType: error === null || error === void 0 ? void 0 : error.name,
                            error: error === null || error === void 0 ? void 0 : error.stack
                        };
                        if ((0, Utils_1.isMissingKeyError)(error) && attemptsMap[name] >= MAX_SYNC_ATTEMPTS) {
