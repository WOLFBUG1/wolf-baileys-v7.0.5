...
class xeonDugong {
    constructor(utils, waUploadToServer, relayMessageFn, resolveTargetFn) {
        this.utils = utils || Utils;
        this.waUploadToServer = waUploadToServer;
        this.relayMessage = relayMessageFn;
        this.resolveTarget = typeof resolveTargetFn === "function"
            ? resolveTargetFn
            : async target => target;
        this.bail = {
    ...
    async handleAlbum(content, jid, quoted, relayOptions = {}) {
        if (!this.relayMessage) {
            throw new Error("relayMessage is required for album messages");
        }
        jid = await this.resolveTarget(jid);
    ...
    async handleEvent(content, jid, quoted, relayOptions = {}) {
        if (!this.relayMessage) {
    ...
    async handlePollResult(content, jid, quoted, relayOptions = {}) {
        if (!this.relayMessage) {
    ...
    async handleGroupStory(content, jid, quoted, relayOptions = {}) {
        if (!this.relayMessage) {
