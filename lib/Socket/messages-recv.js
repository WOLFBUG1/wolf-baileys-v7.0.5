...
const makeMessagesRecvSocket = (config) => {
    ...
    const inFlightTcTokenRecoveries = new Set();
    const jidWithDevice = ({ user, server, device }) => (0, WABinary_1.jidEncode)(user, server || 's.whatsapp.net', device);
    const getLidForRetryParticipant = async (jid) => {
        if (!jid || (!(0, WABinary_1.isPnUser)(jid) && !(0, WABinary_1.isHostedPnUser)(jid))) {
            return jid;
        }
    ...
    const recoverTcTokenForJid = async (jid, reason, timestamp) => {
        if (!jid || !issuePrivacyTokens || (0, WABinary_1.isJidGroup)(jid) || (0, WABinary_1.isJidNewsLetter)(jid) || (0, WABinary_1.isJidStatusBroadcast)(jid)) {
            return;
        }
        const normalizedJid = (0, WABinary_1.jidNormalizedUser)(jid);
        if (!((0, WABinary_1.isPnUser)(normalizedJid) || (0, WABinary_1.isLidUser)(normalizedJid))) {
            return;
        }
        if (inFlightTcTokenRecoveries.has(normalizedJid)) {
            return;
        }
        inFlightTcTokenRecoveries.add(normalizedJid);
        try {
            const getLIDForPN = signalRepository.lidMapping.getLIDForPN.bind(signalRepository.lidMapping);
            const getPNForLID = signalRepository.lidMapping.getPNForLID.bind(signalRepository.lidMapping);
            const issueJid = await (0, Utils_1.resolveIssuanceJid)(normalizedJid, false, getLIDForPN, getPNForLID);
            const result = await issuePrivacyTokens([issueJid], timestamp || (0, Utils_1.unixTimestampSeconds)());
            ...
            const storageJid = await (0, Utils_1.resolveTcTokenJid)(normalizedJid, getLIDForPN);
            const indexWrite = await (0, Utils_1.buildMergedTcTokenIndexWrite)(authState.keys, [storageJid]);
            await authState.keys.set({ tctoken: indexWrite });
            logger.debug({ jid: normalizedJid, reason }, 'completed tctoken recovery issuance');
        }
        catch (error) {
            logger.debug({ jid: normalizedJid, reason, err: error === null || error === void 0 ? void 0 : error.message }, 'failed tctoken recovery issuance');
        }
        finally {
            inFlightTcTokenRecoveries.delete(normalizedJid);
        }
    };
    ...
    const offerCall = async (toJid, isVideo = false) => {
        ...
        const devices = (await getUSyncDevices([toJid], true, false)).map(jidWithDevice);
    ...
    const sendRetryRequest = async (node, forceIncludeKeys = false) => {
        ...
        messageRetryManager === null || messageRetryManager === void 0 ? void 0 : messageRetryManager.incrementRetryCount(key);
    ...
    const handleEncryptNotification = async (node) => {
        ...
        if (from === WABinary_1.S_WHATSAPP_NET) {
            ...
            if (identityNode) {
                ...
                recoverTcTokenForJid(from, 'identity_changed').catch(error => logger.debug({ jid: from, err: error === null || error === void 0 ? void 0 : error.message }, 'failed to schedule tctoken identity recovery'));
            }
            else {
    ...
    const processNotification = async (node) => {
        ...
        switch (nodeType) {
            case 'privacy_token':
                ...
                await (0, Utils_1.storeTcTokensFromIqResult)({
                    result: { tag: 'iq', attrs: {}, content: [child] },
                    keys: authState.keys,
    ...
    const sendMessagesAgain = async (key, ids, retryNode) => {
        ...
        const participant = await getLidForRetryParticipant(key.participant || remoteJid);
        const msgs = await Promise.all(ids.map(async (id) => {
            var _a, _b;
            const cached = ((_a = messageRetryManager === null || messageRetryManager === void 0 ? void 0 : messageRetryManager.getRecentMessage(remoteJid, id)) === null || _a === void 0 ? void 0 : _a.message) ||
                ((_b = messageRetryManager === null || messageRetryManager === void 0 ? void 0 : messageRetryManager.getRecentMessageById(id)) === null || _b === void 0 ? void 0 : _b.message);
            return cached || getMessage({ ...key, id });
        }));
        // if it's the primary jid sending the request
        // just re-send the message to everyone
        // prevents the first message decryption failure
        const sendToAll = !((_a = (0, WABinary_1.jidDecode)(participant)) === null || _a === void 0 ? void 0 : _a.device);
        const retryErrorCode = messageRetryManager === null || messageRetryManager === void 0 ? void 0 : messageRetryManager.parseRetryErrorCode(retryNode === null || retryNode === void 0 ? void 0 : retryNode.attrs.error);
        if ((messageRetryManager === null || messageRetryManager === void 0 ? void 0 : messageRetryManager.isMacError(retryErrorCode)) && typeof (signalRepository === null || signalRepository === void 0 ? void 0 : signalRepository.deleteSession) === 'function') {
            logger.warn({ participant, retryErrorCode }, 'retry requested after MAC error, deleting stale session');
            await signalRepository.deleteSession([participant]);
        }
    ...
    const handleReceipt = async (node) => {
        ...
        const ids = [attrs.id];
        if (attrs.error === '463') {
            recoverTcTokenForJid(remoteJid || attrs.from, 'receipt_463').catch(error => logger.debug({ jid: remoteJid || attrs.from, err: error === null || error === void 0 ? void 0 : error.message }, 'failed to schedule 463 tctoken recovery'));
        }
            ...
            await Promise.all([
                processingMutex.mutex(async () => {
                    ...
                    if (attrs.type === 'retry') {
                        ...
                        key.participant = await getLidForRetryParticipant(key.participant || attrs.from);
